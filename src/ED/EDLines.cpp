...
#pragma warning(default : 4996)
  ...
  fprintf(fp, "+=======+===========+=======+=========+=== LINES "
              "=====+========+========+========+========+========+========+\n");
  fprintf(fp, "| LineNo| SegmentNo |   a   |    b    |invert|   sx   |   sy   "
...
void JoinCollinearLines(EDLines *lines, double MAX_DISTANCE_BETWEEN_TWO_LINES,
  ...
  while (i < lines->noLines) {
    ...
    for (int j = i + 1; j < lines->noLines; j++) {
      if (lines->lines[j].segmentNo != segmentNo)
        break;
...
#undef PRECISON_ANGLE
  ...
  for (int i = 0; i < lines->noLines; i++) {
    ...
    if (valid) {
      if (i != noValidLines)
        lines->lines[noValidLines] = lines->lines[i];
      noValidLines++;
...
#define USE_GX_GY_PLUS
  ...
  if (smoothingSigma >= 2.0)
    GRADIENT_THRESH += 2; // higher GRADIENT_THRESH for higher smoothing so that
                          // we do not get any false detections
  ...
  if (lines->MIN_LINE_LEN < 9)
    lines->MIN_LINE_LEN = 9;
