...
int Stag::detectMarkers(Mat inImage) {
  ...
  for (int indQuad = 0; indQuad < quads.size(); indQuad++) {
    ...
    Codeword c = decodeFromImage(quads[indQuad]);
    int shift;
    int id;
...
void Stag::logResults(string path) {
  ...
  if (keepLogs)
    drawer.drawQuads(path + "5_distorted quads.png", image,
                     quadDetector.getDistortedQuads());
  drawer.drawMarkers(path + "6_markers.png", image, markers);
  if (keepLogs)
    drawer.drawQuads(path + "7_false quads.png", image, falseCandidates);
  drawer.drawEllipses(path + "8_ellipses.png", image, markers);
}

Codeword Stag::decodeFromImage(const Quad &q) {
